...
public class CoalstoneObliteratorRecipe implements IRecipe<IInventory>
{
    public static final CoalstoneObliteratorRecipe.Serializer SERIALIZER = new CoalstoneObliteratorRecipe.Serializer();
    ...
    public CoalstoneObliteratorRecipe(ResourceLocation id, int recipeWidth, NonNullList<Ingredient> recipeItems, ItemStack recipeOutput, float experience, int cookTime)
    {
        this.id = id;
    ...
    {
        if(inv instanceof CoalstoneObliteratorTileEntity)
        {
            CoalstoneObliteratorTileEntity obliterator = (CoalstoneObliteratorTileEntity) inv;

            for(int inputIndex = 0; inputIndex < CoalstoneObliteratorTileEntity.INPUT_SLOT_COUNT; inputIndex++)
            {
                int obliteratorTestSlot = CoalstoneObliteratorTileEntity.FIRST_INPUT_SLOT_INDEX + inputIndex;
                boolean ingredientTest = this.recipeItems.get(inputIndex).test(obliterator.getStackInSlot(obliteratorTestSlot));
                if(!ingredientTest)
                {
                    return false;
                }
    ...
    {
        return new ItemStack(BlockRegistry.COALSTONE_OBLITERATOR.get());
    }

    @Override
    public IRecipeType<?> getType()
    {
        return RecipeRegistry.COALSTONE_OBLITERATOR_RECIPE_TYPE;
    }
    ...
    private static class Serializer extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<CoalstoneObliteratorRecipe>
    {
        Serializer()
        {
            this.setRegistryName(new ResourceLocation("coalstone", "coalstone_obliterator_recipe"));
        }

        @Override
        public CoalstoneObliteratorRecipe read(ResourceLocation recipeId, JsonObject json)
        {
            ...
            return new CoalstoneObliteratorRecipe(recipeId, patternWidth, ingredientList, output, experience, cookTime);
        }

        @Override
        public CoalstoneObliteratorRecipe read(ResourceLocation recipeId, PacketBuffer buffer)
        {
            ...
            return new CoalstoneObliteratorRecipe(recipeId, recipeWidth, ingredientList, output, experience, cookTime);
        }

        @Override
        public void write(PacketBuffer buffer, CoalstoneObliteratorRecipe recipe)
        {
